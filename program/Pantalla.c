...
Imagen lee_imagen(char *fichero, int transparencia)
{
    ...
    if (transparencia)
    {
        ...
        else
        {
            ...
            {
            case 15:
                if (SDL_SetColorKey(surface, SDL_TRUE, (*(Uint16 *) surface->pixels) & 0x00007FFF)==-1) error_pantalla();
                break;
            case 16:
                if (SDL_SetColorKey(surface, SDL_TRUE, *(Uint16 *) surface->pixels)==-1) error_pantalla();
                break;
            case 24:
                if (SDL_SetColorKey(surface, SDL_TRUE, (*(Uint32 *) surface->pixels) & 0x00FFFFFF)==-1) error_pantalla();
                break;
            case 32:
                if (SDL_SetColorKey(surface, SDL_TRUE, *(Uint32 *) surface->pixels)==-1) error_pantalla();
                break;
            }
        }
    }
    ...
    if (SDL_HasColorKey(surface) && !SDL_HasColorKey(imagen->surface))
    {
        ...
        else
        {
            ...
            {
            case 15:
                if (SDL_SetColorKey(imagen->surface, SDL_TRUE, (*(Uint16 *) imagen->surface->pixels) & 0x00007FFF)==-1) error_pantalla();
                break;
            case 16:
                if (SDL_SetColorKey(imagen->surface, SDL_TRUE, *(Uint16 *) imagen->surface->pixels)==-1) error_pantalla();
                break;
            case 24:
                if (SDL_SetColorKey(imagen->surface, SDL_TRUE, (*(Uint32 *) imagen->surface->pixels) & 0x00FFFFFF)==-1) error_pantalla();
                break;
            case 32:
                if (SDL_SetColorKey(imagen->surface, SDL_TRUE, *(Uint32 *) imagen->surface->pixels)==-1) error_pantalla();
                break;
            }
        }
    }
